...
import { View, Text, Button, StyleSheet, TouchableOpacity, TextInput, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
export const ExpenseForm: React.FC<FooProps> = (props) => {
  ...
  const saveExpense = () => {
    ...
    createSingleDoc(updatedAmount, date, async (insertId: number) => {
      const images = await insertImages(tempPhotos.map(uri => uri.uri), insertId!)
      addData({ id: insertId!, amount: updatedAmount, date, photoUri: images })
      bootstrapTmpPhotos([]);
      Haptics.notificationAsync(
  ...
  const retrievePhoto = async () => {
    ...
    try {
      ...
      if (!result.canceled) {
        bootstrapTmpPhotos(result.assets.map((asset, index) => ({
          uri: asset.base64!,
          id: index
        })));
      }
  ...
  return (
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={
        ...
        {monthPaid ? <AnimatedGIF title='Already paid this month.' unicode={EMOJIS.DONALD_MONEY} url={require('../assets/money.gif')} /> :
          ...
          <>
            ...
            <View style={{ flex: 1, display: 'flex', justifyContent: 'flex-end' }}>
              ...
              < View style={styles.searchSection}>
                <MaterialCommunityIcons name="calendar-today" size={25} color={'blue'} />
                <TextInput
                  placeholder="Date"
                  value={date}
                  editable={false}
              ...
              <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                ...
                {tempPhotos.length > 0 &&
                  <View style={{ display: 'flex', flexDirection: 'row', gap: 10, alignItems: 'center' }}>
                    ...
                    <MaterialCommunityIcons name="eye-settings-outline" color={'gray'} size={23} onPress={() => props.navigation.navigate('TempImages')} />
                  </View>
                }
              </View>
            </View>
            ...
            <View style={{
              marginTop: 20, flex: 1, display: 'flex',
